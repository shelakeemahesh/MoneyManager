...
public class ProfileController {
    ...
    public ResponseEntity<ApiResponse<ProfileDTO>> registerProfile(@Valid @RequestBody ProfileDTO profileDTO) {
        ProfileDTO registered = profileService.registerProfile(profileDTO);
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.success(registered, "Registration successful. Please check your email to activate."));
    }
    ...
    @GetMapping("/activate")
    public ResponseEntity<ApiResponse<Map<String, String>>> activateAccount(@RequestParam String token) {
