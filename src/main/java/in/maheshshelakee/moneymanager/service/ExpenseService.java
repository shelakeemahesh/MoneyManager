...
public class ExpenseService {
    ...
    public ExpenseResponse add(ExpenseRequest request, String email) {
        ...
        if (!categoryRepository.existsByNameAndTypeAndProfile(catName, "EXPENSE", profile)) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Invalid category: " + catName);
        }
    ...
    public ExpenseResponse update(Long id, ExpenseRequest request, String email) {
        ...
        String catName = request.getCategory().trim();
        if (!entity.getCategory().equalsIgnoreCase(catName)) {
            if (!categoryRepository.existsByNameAndTypeAndProfile(catName, "EXPENSE", profile)) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Invalid category: " + catName);
            }
        }
